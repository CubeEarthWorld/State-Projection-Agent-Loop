"""Discovery search engine (§5 layers 2–3, §9): BM25, vector mixing,
no_embed exclusion, category filters, epoch-driven reindexing."""
...
class TestLexical:
    def test_japanese_query_hits_embedding_text(self):
        ...
        assert results[0].tool.name == "web_search"
    ...
    def test_exact_name_query_ranks_first(self):
        ...
        assert results[0].tool.name == "play_bgm"
...
class TestLayers:
    ...
    def test_exclude_set(self):
        search = ToolSearch(make_registry(), vector="off")
        names = [s.tool.name for s in search.search("検索して", exclude={"web_search"}, k=8)]
...
class TestCategoryFilter:
    def test_prefix_match(self):
        ...
        assert {s.tool.name for s in results} == {"web_search"}

    def test_exact_match(self):
        search = ToolSearch(make_registry(), vector="off")
        results = search.search("BGM 音楽", category="game/media", k=8)
        assert [s.tool.name for s in results] == ["play_bgm"]
...
class TestReindexing:
    def test_new_tool_found_after_epoch_bump(self):
        ...
        assert not any(s.tool.name == "translate" for s in search.search("翻訳 translate", k=8))
        reg.register(tool_dict("translate", summary="テキストを翻訳する",
                               embedding_text="翻訳して 英語に 日本語に translate"))
        results = search.search("翻訳して translate", k=8)
        assert results[0].tool.name == "translate"
