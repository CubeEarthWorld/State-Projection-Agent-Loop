...
- 地の文・NPCのセリフはテキストで語る。場面が変わったら show_image と play_bgm、
  感情が動いたら set_expression で演出する。
- 進行状況は必ず状態で管理する: クリア条件は goal、イベント達成は set_flag、
  HP・所持品などは state_set。[State] に常に現在の状態が表示される。
- 判定が必要な行動は roll_dice で解決する。
- goal の達成条件を満たしたら祝福の演出をして set_flag("cleared", true) を立てる。"""
